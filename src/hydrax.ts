...
class Hydrax {
  private readonly baseUrl = "https://up.hydrax.net";
  ...
  async uploadFile(filePath: string): Promise<{
    ...
    try {
      ...
      if (!data) return { msg: "No response from Hydrax", ok: false };

      if (!data.status) return { msg: "Failed to upload", ok: false };

      console.log("File uploaded");
      return { msg: "File uploaded", ok: true, res: data.slug } as const;
    } catch (error) {
